...
- **Calm film:** the export has only one jacket clip, containing rapid zooms and a cut. `prepare_calm_jacket.py` edits its stable 3.55–4.25s ending into a slowed forward/reverse loop with a matching poster. The original is untouched, and `calm-jacket-edit.json` records the edit. `validate_calm_jacket.py` rejects the original jumpy clip and passes the calm one (sampled peak frame change 89.14 → 0.79; reset seam 59.93 → 0.83).
- **Video contrast / pill shadow:** jacket title and over-video navigation labels are white. White product cards and the selected white For You pill retain dark text. The shared topic rail now has 32pt of real shadow space above/below its 40pt controls, with a soft leading mask rather than a hard cutoff. Before/after Simulator checks retained the For You frame at x62/y66, 77×40pt.
