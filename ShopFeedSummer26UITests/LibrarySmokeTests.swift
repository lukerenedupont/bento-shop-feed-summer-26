...
final class LibrarySmokeTests: XCTestCase {
    ...
    func testPriceResearchOpensFirstWithOffersAndPreservesNativeJourney() {
        ...
        scrollTo(app.staticTexts["More from running shops."], in: app, attempts: 10)
        XCTAssertTrue(app.descendants(matching: .any)["research.merchant-card.gid://shopify/Shop/7546175546"].waitForExistence(timeout: 5))
        capture(app, name: "Shop Agent — running merchant cards")
