...
- Latest Simulator acceptance: all fifteen tests pass (`/tmp/generative-first-cards-acceptance.log`). Added coverage for exact saved pairs after swap/regeneration, comparison focus and prices, canonical destination links and consumer feed geometry/real-shell actions.
...
- Legacy `Media/` films remain excluded from this branch target, not deleted. The original `184320 KB` gate is unchanged; this build is approximately `181396 KB`.
