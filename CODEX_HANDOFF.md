...
- `main` at `c92a7dc` is the confirmed visual/behavior reference; it remains untouched. The target is twenty genuinely useful, visually distinct commerce experiences in the native flick-and-stick feed. User approval still gates visual quality: a passing build or twenty rendered cards is not approval.
- Standards Manual / Graphic standards remains approved. The subsequent outfit/chair styling was explicitly rejected. Those six-job experiments are retained only behind `-legacyGenerativeJobs`, not used as the new default sequence.
- The default now contains 20 authored scenes, 52 canonical products and 17 merchants. `EDITORIAL_PLANS.json` → `EditorialFeedCatalog.swift` → `EditorialCommerceFeedCard.swift` is the new bounded native composition path; the approved books renderer remains separate. These are authored examples of generative output, not runtime AI generation. Regeneration revalidates their catalog references.
...
- Current app budget: 184096 / 184320 KB. Home remains below its 1900-line limit. Keep additions lean.
