...
## Current unified demo — 2026-09-09

- Shareable snapshot for GitHub branch `next-generation-feed-20-cards`. The tested implementation was transferred onto `6563dba` in a clean publishing worktree, without carrying unrelated review-checkout changes. `UNIFIED_DEMO.md` is the current walkthrough and implementation guide. The original review checkout remains separately dirty; the older checkout/status notes below are historical.
- Run `./Scripts/run_unified_demo.sh [room|footwear|books]`. It opens the actual feed on Next Generation QA (`3F187DB4-1903-4F9D-A567-9779113B8229`), not a new gallery shell. All twenty cards remain available.
- Room → eligible chair comparison → chosen chair returned to the same room; footwear direction → selected Salomon bound into a product-led outfit; library → exact book inspection/keep/resume. The separately approved editorial book introduction was not ported or approximated. FOROM/Lichen are still independent cards, not fully connected child journeys.
- `FeedJourneyMemory.swift` is the Foundation-only snapshot domain. `DemoJourneyCatalog.swift` and `DemoJourneyActions.swift` own authored transitions, entity binding, and scoped resume. `JourneySelectionViews.swift` adds explicit keep, continuation actions and one shared Kept selections destination. The existing session optionally persists NG20 state under `unifiedFeedDemo.luke.v1`; reset only removes that namespace. Other prototypes can still use ephemeral sessions.
- `build_next_generation_20.py` owns two new `journeyTemplates` beside the same twenty cards. No additional media. The core validates template structure; the journey adapter validates bound roles/artwork. The inspector now exports the actual resolved composition, including continuation bindings.
- Native tests exposed offscreen footer hit targets clamped over the current card. Removed the NG20 scroll-relative footer visualEffect and bounded each card's hit shape. Home journey navigation synchronizes the native row and position cache, tolerates startup layout shifts, and releases on user tracking/scrolling. Consumer startup also now uses the key window/status-bar inset before foregroundActive and initializes its journey target before the first utility layout, preventing footer/nav overlap and wrong-card startup. No debug instrumentation remains.
- Final verification: **12 model + 5 UI tests passed**, including native room navigation, bottom clearance, ordinary flick/next-card activation, footwear relaunch/resume, book keep, and consumer-mode startup/relaunch clearance (`/tmp/unified-final-acceptance.log`). **10 CLI contract checks**, authoring parity and provenance passed (`/tmp/unified-final-contract.log`). Normal build **173220 / 184320 KB**, Home **1848 / 1900 lines** (`/tmp/unified-final-build.log`). `git diff --check` passed. UI tests use a separate defaults suite and do not erase the presenter's saves.
- Full twenty-card visual approval, physical AR, backend persistence and live generation remain unverified/unimplemented. Earlier cleanup/test blockers below are historical; hosted integration tests now pass with the explicit test-host flag and cached dependencies.

## Current cleanup — review checkout, schema 2

- Work remains in `/Users/lukedupont/bento-shop-feed-review`, `review/quiet-shopping-grammar`. Preserve all unrelated dirty work. No cleanup changes have been committed or pushed.
- `Packages/ShopCompositionCore` holds the typed, UI-independent tree and structural validator; the app adds asset/commerce checks. Schema 2 separates layout, product presentation, media fit/playback, and card action/disclosure. Unknown node fields fail decoding with a path; validators return field-specific issues. The twenty fixtures were verified equal to the old payload after only the schema transformation before rewriting them.
- `CompositionContext` now resolves product bindings into dictionaries per context, reused by child nodes and swaps. Session behavior remains app-owned; this is not persistent World-state extraction.
- Edit the Python authoring source, then use `build_next_generation_20.py --write-spec`. `--check-spec` rejects drift without downloads or writes; the review launcher runs it automatically. Do not manually patch bundled JSON anymore.
- `Scripts/check_composition_contract.sh` combines deterministic authoring, provenance and ten CLI contract regressions using the same Swift core as the app. All ten passed through a Simulator runner (`/tmp/composition-contract-acceptance.log`). This machine kills native macOS test/CLI processes under its security policy; the optional `--simulator <booted UDID>` runner needs no app host and changes no security settings.
- Final combined checks passed (`/tmp/composition-cleanup-final-checks.log`): reproducibility, provenance and ten actual-core CLI regressions. Normal app build passed at 172532 KB (`/tmp/composition-cleanup-final-build.log`); schema-2 Vomero card rendered in Next Generation QA (`/tmp/composition-cleanup-vomero.png`). The hosted test attempt in separate DerivedData timed out after dependency resolution; no fresh hosted test success is claimed. Full interaction/visual regressions are pending.
- Read `NEXT_GENERATION_20.md` and the core package README for the new seam and remaining production-scale work. This pass does not consolidate the two prototype branches, implement live AI, persistent state or paged delivery.
