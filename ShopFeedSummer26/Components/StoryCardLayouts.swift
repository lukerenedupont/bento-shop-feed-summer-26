    ...
    var body: some View {
        ZStack {
            if items.indices.contains(0) {
                floatingProduct(items[0], width: 190, height: 230, rotation: -5)
                    .offset(x: -62, y: 8)
            }
            if items.indices.contains(1) {
                floatingProduct(items[1], width: 155, height: 180, rotation: 5)
                    .offset(x: 80, y: -55)
            }
            if items.indices.contains(2) {
                floatingProduct(items[2], width: 135, height: 145, rotation: 3)
                    .offset(x: 88, y: 105)
            }
            if items.indices.contains(3) {
                floatingProduct(items[3], width: 110, height: 120, rotation: -7)
                    .offset(x: -92, y: 150)
            }
        }
